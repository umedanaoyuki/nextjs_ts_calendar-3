...
import { CalenderBody } from "../organisms/CalendarBody";
import { useCalendar } from "@/app/hooks/useCalendar";
import { useEffect, useState } from "react";
import { CalendarNav } from "../organisms/CalendarNav";
import { DateList, Schedule } from "@/app/types/calendar";
import { getScheduleList } from "@/app/api/calendar";
...
export const CalendarPage = () => {
  ...
  return (
    <>
      <h1 className="font-bold text-3xl mb-5 mt-20 text-center">{`${
        getMonth(currentDate) + 1
      }月`}</h1>
      <CalendarNav setCurrentDate={setCurrentDate} addSchedule={addSchedule} />
      <table className="w-[80%] border-collapse border-2 border-solid border-lime-800 table-fixed mx-auto">
        <CalendarHeader />
        <CalenderBody
          currentDate={currentDate}
          dateList={dateList}
